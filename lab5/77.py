...
rectangle(0,0,470,600)            # 1
brushColor(0,0,0)
penColor(0,0,0)
rectangle(0,280,470,700)                # 2

brushColor(255, 255, 255)#луна
penColor(150, 150, 150)
circle(390, 60, 40)                       # 3 

def samaya_yzhasnaya_fignya (X,Y): #МАЛЕНЬКИЕ СЛЕВА
    brushColor(179,179,179)
    penColor(179,179,179)
    circle(X,Y,12)
    polygon([(X - 12, Y), (X - 28, Y + 25), (X - 25, Y + 20), (X - 20, Y + 30), (X - 15, Y + 38), (X - 10, Y + 25),
             (X, Y + 30), (X + 10, Y + 33), (X + 15, Y + 25), (X + 20, Y + 20), (X + 12, Y)])
...
def house(x,y):
    ...
    brushColor(randColor()) #окна new_______________________________________________________
    penColor(72,62,55)
    rectangle(x+12,y + 100,x+27,y + 130)
    brushColor(randColor())
    rectangle(x+39,y + 100,x+54,y + 130)
    brushColor(randColor())
    rectangle(x+66,y + 100,x+81,y + 130)
    brushColor(randColor())
    rectangle(x+93,y + 100,x+108,y + 130)
...
#ПРИВИДЕНИЯ О УЖАС
samaya_yzhasnaya_fignya(405,360)

samaya_yzhasnaya_fignya(425,390)
samaya_yzhasnaya_fignya(350,460)

samaya_yzhasnaya_fignya(80,500)
samaya_yzhasnaya_fignya(100,520)
...
brushColor(179,179,179)
penColor(179,179,179)
polygon([(X-R,Y),(X-R-40,Y+40),(X-R-20,Y+35),(X-R-10,Y+40),(X-R+15,Y+50),(X-R+30,Y+30),(X-R+50,Y+35),(X-R+60,Y+40),(X+R,Y)])
circle(X,Y,R)
brushColor(135,202,222)
penColor(135,202,222)
circle(X-10,Y,4)
...
def update():
    for z in range(6):
        a = randint(0, 360)
        for i in range(88 + z * 6)[(82 + z * 6):(88 + z * 6)]:
            moveObjectBy(i, 5 * math.cos(a), 5 * math.sin(a))

fps = 24
onTimer(update, 1000 //fps)
run()
